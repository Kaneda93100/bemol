...
# Preload bundled dependencies so QBlade.so can find them
for lib in ["libQGLViewer.so.2", "libxlnt.so.1.5.0", "libChronoEngine.so", "libclblast.so.1"]:
    CDLL(os.path.join(lib_dir, lib), mode=RTLD_GLOBAL)
...
dll_directory = "/home/arthur/Bureau/QBladeCE_2.0.9.7_unix/QBladeCE_2.0.9.7/SIL_Interface"
...
QBLADE.loadSimDefinition(os.path.join(sil_dir, "OC4_Semi_Test.sim").encode())
#QBLADE.loadProject(os.path.join(sil_dir, "NREL_5MW_Sample.qpr").encode())
...
QBLADE.storeProject(os.path.join(sil_dir, "NREL_5MW_Sample_completed.qpr").encode())

# Storing the simulation results in QBlade ASCII format in the file NREL_5MW_Sample_results.txt
QBLADE.exportResults(0, sil_dir.encode() + b"/", b"NREL_5MW_Sample_results", b"")
